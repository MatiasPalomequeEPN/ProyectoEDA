...
public class ColaDobleSalidaRestringidaPanel extends JPanel implements AnimacionInteractiva {

    private static final int CAPACIDAD = 6;

    /** Pseudocodigo combinado (ColaDVacia + ColaDLlena + InsertarDerecha + InsertarIzquierda + Eliminar), numeracion global 1..N. */
    public static final String[] PSEUDOCODIGO = {
        ...
        " ",
        "ALGORITMO InsertarDerecha(COLA, frente, fin, MAX, B, D)",
        "  COLA = Arreglo[0..5] de enteros",
        "  Llamar ColaDLlena()",
        ...
        "    Fin Si",
        "  Fin Si",
        "Fin Algoritmo InsertarDerecha()",
        " ",
        "ALGORITMO InsertarIzquierda(COLA, frente, fin, MAX, B, D)",
        "  COLA = Arreglo[0..5] de enteros",
        "  Si (frente == 0) entonces",
        "    Escribir \"Desbordamiento por la izquierda\"",
        "  sino",
        "    Si (frente == -1) entonces",
        "      Hacer frente <- 0",
        "      Hacer fin <- 0",
        ...
        "    Hacer COLA[frente] <- D",
        "  Fin Si",
        "Fin Algoritmo InsertarIzquierda()",
        " ",
        "ALGORITMO Eliminar(COLA, frente, fin, MAX, B, Dato)",
        "  COLA = ARREGLO[0..5] de enteros",
    ...
    public static final String[] CODIGO = {
        ...
        "}",
        " ",
        "public boolean insertarDerecha(int valor) {",
        ...
        "}",
        " ",
        "public boolean insertarIzquierda(int valor) {",
    ...
    private final JButton btnInsertarIzq = new JButton("Insertar Izq.");
    private final JButton btnInsertarDer = new JButton("Insertar Der.");
    private final JButton btnEliminar = new JButton("Eliminar (Frente)");

    public ColaDobleSalidaRestringidaPanel(Color colorTema) {
        ...
        controles.add(btnInsertarIzq);
        controles.add(btnInsertarDer);
        ...
        btnInsertarIzq.addActionListener(e -> intentarInsertar(true));
        btnInsertarDer.addActionListener(e -> intentarInsertar(false));
    ...
    private List<Paso> generarPasosInsertarDerecha(int valor) {
        List<Paso> r = new ArrayList<>();
        Integer[] copia = arreglo.clone();
        int f = frente, fi = fin;
        String op = "INSERTAR DERECHA(" + valor + ")";

        r.add(new Paso(copia.clone(), f, fi, 19, 9, "Iniciando InsertarDerecha(" + valor + ")", op, null));
        ...
        if (fi == CAPACIDAD - 1) { 
            r.add(new Paso(copia.clone(), f, fi, 23, 10, "Desbordamiento: no se puede insertar " + valor + " por la derecha", op, null));
            return r;
        }
        ...
        r.add(new Paso(copia.clone(), f, fi, 33, 16, "Fin Algoritmo InsertarDerecha()", op, fi));
    ...
    private List<Paso> generarPasosInsertarIzquierda(int valor) {
        List<Paso> r = new ArrayList<>();
        Integer[] copia = arreglo.clone();
        int f = frente, fi = fin;
        String op = "INSERTAR IZQUIERDA(" + valor + ")";

        r.add(new Paso(copia.clone(), f, fi, 35, 19, "Iniciando InsertarIzquierda(" + valor + ")", op, null));
        r.add(new Paso(copia.clone(), f, fi, 37, 20, "¿frente == 0? -> " + (f == 0), op, null));

        if (f == 0) { 
            r.add(new Paso(copia.clone(), f, fi, 38, 20, "Desbordamiento: no hay espacio a la izquierda para " + valor, op, null));
            return r;
        }

        r.add(new Paso(copia.clone(), f, fi, 40, 21, "¿frente == -1? -> " + (f == -1), op, null));

        if (f == -1) {
            f = 0;
            fi = 0;
            r.add(new Paso(copia.clone(), f, fi, 41, 22, "Hacer frente <- 0, fin <- 0 (primer elemento)", op, null));
        } else {
            f = f - 1;
            r.add(new Paso(copia.clone(), f, fi, 44, 24, "Hacer frente <- frente - 1  ->  frente = " + f, op, null));
        }

        copia[f] = valor;
        r.add(new Paso(copia.clone(), f, fi, 46, 26, "Hacer COLA[" + f + "] <- " + valor, op, f));

        r.add(new Paso(copia.clone(), f, fi, 48, 28, "Fin Algoritmo InsertarIzquierda()", op, f));
    ...
    private List<Paso> generarPasosEliminar() {
        List<Paso> r = new ArrayList<>();
        Integer[] copia = arreglo.clone();
        int f = frente, fi = fin;
        String op = "ELIMINAR()";

        r.add(new Paso(copia.clone(), f, fi, 50, 30, "Iniciando Eliminar()", op, null));
        r.add(new Paso(copia.clone(), f, fi, 52, 31, "¿Esta vacia? (frente != -1 -> " + (f != -1) + ")", op, null));

        if (f == -1) { 
            r.add(new Paso(copia.clone(), f, fi, 61, 41, "Subdesbordamiento: la cola esta vacia", op, null));
            return r;
        }

        r.add(new Paso(copia.clone(), f, fi, 53, 32, "Hacer Dato <- COLA[" + f + "] (" + copia[f] + ")", op, f));
        copia[f] = null;

        r.add(new Paso(copia.clone(), f, fi, 54, 34, "¿frente == fin? (" + f + " == " + fi + " -> " + (f == fi) + ")", op, f));

        if (f == fi) { 
            f = -1;
            fi = -1;
            r.add(new Paso(copia.clone(), f, fi, 55, 35, "Hacer frente <- -1, fin <- -1 (cola quedo vacia)", op, null));
        } else {
            f = f + 1;
            r.add(new Paso(copia.clone(), f, fi, 58, 37, "Hacer frente <- frente + 1  ->  frente = " + f, op, null));
        }

        r.add(new Paso(copia.clone(), f, fi, 63, 40, "Fin Algoritmo Eliminar()", op, null));
    ...
    private void intentarInsertar(boolean izquierda) {
        ...
        campoDato.setText("");
        ejecutarOperacion(izquierda ? generarPasosInsertarIzquierda(valor) : generarPasosInsertarDerecha(valor));
    }

    private void intentarEliminar() {
        if (timer.isRunning()) return;
        ejecutarOperacion(generarPasosEliminar());
    }
    ...
    private void actualizarBotones() {
        ...
        btnInsertarIzq.setEnabled(!animando);
        btnInsertarDer.setEnabled(!animando);
        btnEliminar.setEnabled(!animando);
    ...
    private class Lienzo extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.drawString("Cola Doble - Salida Restringida (capacidad " + CAPACIDAD + ")", 15, 22);
            ...
            for (int i = 0; i < CAPACIDAD; i++) {
                ...
                if (i == f) etiqueta(g2, x, y - 12, cajaAncho, "frente (entra/sale)", new Color(200, 60, 60));
                if (i == fi) etiqueta(g2, x, y + cajaAlto + 30, cajaAncho, "fin (solo entra)", new Color(30, 110, 190));
            }
